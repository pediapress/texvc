...
let cmd_convert tmpprefix finalpath resolution_in_dpi = "convert -quality 100 -density " ^ (string_of_int resolution_in_dpi) ^ " " ^ tmpprefix ^ ".ps " ^ finalpath ^ " >/dev/null 2>/dev/null"
...
let cmd_dvipng tmpprefix finalpath resolution_in_dpi = "dvipng -gamma 1.5 -D " ^ (string_of_int resolution_in_dpi) ^ " -T tight --strict " ^ tmpprefix ^ ".dvi -o " ^ finalpath ^ " >/dev/null 2>/dev/null"

exception ExternalCommandFailure of string

let render tmppath finalpath outtex md5 resolution_in_dpi =
	...
	else if (Sys.command (cmd_dvipng tmpprefix (finalpath^"/"^md5^".png") resolution_in_dpi)  != 0)
